...
public:
	...
	void addEdge(int origin, int destination, unsigned int idx); //fills in the spot in the array (x, y) with (origin, destination)
	...
	std::vector< std::vector<int> > m_matrixWithoutIndices;	//adjacency matrix with just 1's and 0's
	std::vector< std::vector<int> > m_matrixContainingIndices;	//the adjacency matrix with numbered values
